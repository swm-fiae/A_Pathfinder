...
for(var y = 0; y < 50; y++)for(let x = 0; x < 50; x++){
...
gridItems.forEach(function(item) {
    item.addEventListener("click", function() {
        ...
        if (previousSpawn) previousSpawn.classList.remove("spawn");
        if (previousEnd) {
...
function addStart() {
    ...
    if (currentStartCell) currentStartCell.classList.remove("spawn");
...
function genMap(percentage) {
    ...
    for(let i = 0; i < 50; i++){
        ...
        for(let j = 0; j < 50; j++){
            ...
            if (randNum < selectedPercentage / 100) value = 1;
            else value = 0;
            row.push(value);
        }
        grid.push(row);
